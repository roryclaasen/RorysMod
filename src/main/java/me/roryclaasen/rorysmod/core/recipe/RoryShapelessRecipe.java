...
public class RoryShapelessRecipe implements IRecipe {
	protected ItemStack output = null;
	protected ArrayList<Object> input = new ArrayList<Object>();

	public RoryShapelessRecipe(Block result, Object... recipe) {
		this(new ItemStack(result), recipe);
	}

	public RoryShapelessRecipe(Item result, Object... recipe) {
		this(new ItemStack(result), recipe);
	}

	public RoryShapelessRecipe(ItemStack result, Object... recipe) {
		output = result.copy();
	...
	public static RoryShapelessRecipe create(NBTTagCompound nbtTags) {
		if (!nbtTags.hasKey("result") || !nbtTags.hasKey("input")) {
			RMLog.error("Shapeless recipe parse error: missing input or result compound tag.");
			return null;
		}
		...
		if (result == null || list.tagCount() == 0) {
			RMLog.error("Shapeless recipe parse error: invalid result stack or input data list.");
			return null;
		}
		...
		for (int i = 0; i < list.tagCount(); i++) {
			...
			if (compound.hasKey("oredict")) {
				...
				RMLog.error("Shapeless recipe parse error: invalid input tag data key.");
				return null;
			}
		}

		return new RoryShapelessRecipe(result, ret);
	}
}