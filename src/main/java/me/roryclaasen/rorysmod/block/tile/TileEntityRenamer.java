...
public class TileEntityRenamer extends TileEntityBase implements ISidedInventory, IEnergyReceiver, ITileInfo {
	...
	public static final int ENERGY_STORAGE = 4000;
	...
	private static final int TICKS_TO_RENAME = 10;
	...
	public void updateEntity() {
		...
		if (input != null) {
			...
			if (energy.getEnergyStored() <= 40 + renamingName.length()) return;

			if (active) {
				tickSinceLastRename++;
				// sendGUIUpdate();
			}
			if (tickSinceLastRename < TICKS_TO_RENAME) return;
	...
	public int getScaledProgress(int paramInt) {
		return tickSinceLastRename * (paramInt / TICKS_TO_RENAME);
	}
	...
	public void setTicksProcessed(int ticks) {
		this.tickSinceLastRename = ticks;
	}
}
